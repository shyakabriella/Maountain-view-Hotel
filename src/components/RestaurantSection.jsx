...
const MenuItem = ({ item }) => (
  ...
  >
    <div className="flex items-start justify-between gap-4">
      <div className="flex-1">
        <h4 className="text-[15px] font-medium text-[#183247] transition-colors group-hover:text-[#2E7D32]">
          {item.name}
        </h4>
...
const RestaurantSection = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${API_URL}/home-section7`);
      const result = await response.json();
      if (result.success && result.data) {
        setSectionData({
          section_title: result.data.section_title || "Menu",
          categories: result.data.categories || []
        });
        if (result.data.categories && result.data.categories.length > 0) {
          setActiveTab(result.data.categories[0].name);
        }
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="bg-[#f3f2ed] min-h-[100vh] py-12 sm:py-15 md:py-20">
        <div className="mx-auto max-w-[1380px] px-5 sm:px-6 md:px-8 lg:px-10">
          <div className="h-12 w-32 bg-gray-200 animate-pulse mx-auto mb-10" />
  ...
  const activeCategory = sectionData.categories.find(cat => cat.name === activeTab);
  const currentItems = activeCategory?.items || [];
  const midpoint = Math.ceil(currentItems.length / 2);
  const leftColumn = currentItems.slice(0, midpoint);
  const rightColumn = currentItems.slice(midpoint);
    ...
    <section className="bg-[#f3f2ed] relative overflow-hidden min-h-[100vh] py-12 sm:py-15 md:py-20">
      <div className="mx-auto max-w-[1380px] px-5 sm:px-6 md:px-8 lg:px-10">
        <motion.div
          ...
          className="text-center"
        >
          <h2 className="text-[30px] font-normal leading-[1] text-[#183247] sm:text-[40px] md:text-[50px]">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="mt-14"
          >
            <h3 className="text-[20px] font-normal leading-[1] text-[#183247] sm:text-[30px] md:text-[40px]">
              {activeTab}
            </h3>
